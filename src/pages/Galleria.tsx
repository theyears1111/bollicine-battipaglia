...
export default function Galleria() {
  ...
  // Dividi in 3 colonne per masonry
    ...
    <>
              ...
              <button key={c} onClick={() => setFiltro(c)}
                className={`font-sans text-xs tracking-widest uppercase px-5 py-2 border transition-all duration-300 ${
                  filtro === c
                    ...
                    : 'border-white/10 text-white/40 hover:border-oro/40 hover:text-oro/60'
                }`}>
                {c}
              </button>
            ))}
            ...
            {[col1, col2, col3].map((col, ci) => (
              <div key={ci} className="flex flex-col gap-3">
                {col.map((foto, fi) => {
                  ...
                  // Alterna proporzioni per effetto mosaic
                  ...
                  return (
                    <div key={fi}
                      ...
                      <img src={foto.url} alt={foto.titolo}
                        ...
                        loading="lazy" />
                      <div className="absolute inset-0 bg-gradient-to-t from-nero/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400" />
                      <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        <p className="font-sans text-xs text-white/80 tracking-wide">{foto.titolo}</p>
                        <p className="font-sans text-xs text-oro/60 mt-1">{foto.categoria}</p>
                      </div>
                    </div>
                  );
                })}
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-nero/97 flex items-center justify-center p-4"
          ...
          <button className="absolute left-4 md:left-8 text-white/50 hover:text-oro transition-colors z-10 text-3xl font-thin"
            onClick={e => { e.stopPropagation(); prev(); }}>‹</button>
          <button className="absolute right-4 md:right-8 text-white/50 hover:text-oro transition-colors z-10 text-3xl font-thin"
            onClick={e => { e.stopPropagation(); next(); }}>›</button>
          <div className="max-w-4xl w-full" onClick={e => e.stopPropagation()}>
            <img src={lightbox.url} alt={lightbox.titolo}
